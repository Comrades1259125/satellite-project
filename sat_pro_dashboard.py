...
# ==========================================
st.set_page_config(page_title="V5950 PRO-COMMAND", layout="wide")

# ป้องกันปัญหาป๊อปอัพเด้งเวลาเลื่อนสไลเดอร์
...
def run_calculation(sat_obj, target_dt=None):
    ...
    # 40 Telemetry Params (ครบตามสั่ง)
    tele = {
    ...
    for p in prefixes:
        for s in ["STAT", "VOLT", "CURR", "TEMP"]:
            if len(tele) < 40:
                tele[f"{p}_{s}"] = f"{random.uniform(12.0, 98.0):.2f}"
...
def build_pdf(sat_name, f_id, pwd, m):
    ...
    pdf.cell(0, 10, f"STRATEGIC MISSION REPORT: {f_id}", ln=True, align='C')
    pdf.ln(10)
...
# ดักเรียก Dialog ไว้นอก Fragment เพื่อป้องกันการเด้งเวลาซูม
...
def dashboard(asset_name):
    ...
    st.markdown(f"<h1 style='text-align:center;'>{datetime.now().strftime('%H:%M:%S')} UTC</h1>", unsafe_allow_html=True)
