...
def build_pdf(sat_name, addr, s_name, s_pos, s_img, f_id, pwd, m_main, m_hist):
    pdf = ULTIMATE_PDF()
    pdf.add_page()
    pdf.set_font("Arial", 'B', 16); pdf.cell(0, 10, "OFFICIAL MISSION ARCHIVE V8.5", ln=True, align='C')
    ...
    for i in range(0, 44, 4):
        for j in range(4):
            if i+j < len(items): pdf.cell(47.5, 7, f"{items[i+j][0]}: {items[i+j][1]}", border=1)
        pdf.ln()

    pdf.ln(5); pdf.draw_graph(10, 105, 190, 40, "LATITUDE TRAJECTORY", m_hist["lats"], (0, 102, 204))
    pdf.draw_graph(10, 155, 90, 35, "VELOCITY (KM/H)", m_hist["vels"], (204, 0, 0))
    pdf.draw_graph(110, 155, 90, 35, "ALTITUDE (KM)", m_hist["alts"], (0, 153, 51))
    
    # QR Code Frame
    pdf.set_draw_color(0); pdf.rect(15, 215, 40, 50)
    ...
    pdf.line(120, 245, 190, 245); pdf.set_xy(120, 247); pdf.set_font("Arial", 'B', 10); pdf.cell(70, 5, s_name.upper() if s_name else "DIRECTOR", align='C', ln=True)
...
# UI & CONTROL
# ==========================================
st.set_page_config(page_title="ZENITH V8.5", layout="wide")
...
with st.sidebar:
    ...
    p_a = st.selectbox("จังหวัด (Sync Time)", list(LOC_DB.keys()))
    d_a = st.text_input("อำเภอ", "Mueang")
    s_a = st.text_input("ตำบล", "That Choeng Chum")
    
    # ปุ่มยืนยันใต้ที่อยู่
    if st.button("✅ CONFIRM LOCATION & SYNC", use_container_width=True):
        st.session_state.loc_confirmed = True
        st.toast(f"System Synced to {p_a} Timezone")
...
def modal():
    if st.session_state.pdf_blob is None:
        ...
        # UI หน้าดาวน์โหลดตามตัวอย่างรูปที่ 1 & 2
        st.markdown(f"""
            <div style="border: 4px solid black; padding: 25px; border-radius: 15px; text-align: center; background: white; color: black;">
                <p style="font-weight: bold; font-size: 14px; color: gray;">DOCUMENT ARCHIVE ID</p>
                <h2 style="color: red; font-weight: 900; font-size: 30px; margin-top: -10px;">{st.session_state.fid}</h2>
                <hr style="border: 1px solid #eee;">
                <p style="font-weight: bold; font-size: 14px; color: gray;">ENCRYPTION KEY</p>
                <h1 style="font-weight: 900; font-size: 55px; letter-spacing: 15px; margin-top: -10px;">{st.session_state.pwd}</h1>
            </div>
        """, unsafe_allow_html=True)
        st.write("")
        st.download_button("📥 DOWNLOAD ENCRYPTED ARCHIVE", st.session_state.pdf_blob, f"{st.session_state.fid}.pdf", use_container_width=True)
...
def main_dashboard():
    # นาฬิกาแบบแคปซูลตามรูปที่ส่งมา
    synced_time = (datetime.now(timezone.utc) + timedelta(hours=st_tz)).strftime("%H:%M:%S")
    st.markdown(f"""
        <div style="text-align:center; background:white; border:5px solid black; border-radius:100px; padding:15px; margin-bottom:20px;">
            <span style="color:black; font-size:70px; font-weight:900; font-family:monospace; line-height:1;">{synced_time}</span>
            <p style="margin:0; font-weight:bold; color:gray; font-size:12px;">LOCATION: {p_a}</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    if sat_catalog and sel_sat in sat_catalog:
        ...
        cols = st.columns(3)
        cols[0].metric("ALTITUDE", f"{m_main['ALT']} KM")
        cols[1].metric("VELOCITY", f"{m_main['VEL']} KM/H")
        cols[2].metric("POSITION", f"{m_main['LAT']}, {m_main['LON']}")
        ...
        st.subheader("📊 40-PARAMETER LIVE TELEMETRY STREAM")
        # แสดงตาราง 40 พารามิเตอร์แบบ 4 คอลัมน์
        all_items = list(m_main.items())
        df_rows = [all_items[i:i+4] for i in range(0, 44, 4)]
