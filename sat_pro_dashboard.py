...
# ==========================================
# 1. CORE DATA ENGINE (NASA TLE DATA)
# ==========================================
@st.cache_resource
def init_system():
    # ดึงพิกัดดาวเทียมจริงจากฐานข้อมูล Celestrak
...
def run_calculation(sat_obj, target_dt=None):
    ...
    # ระบบจำลองข้อมูลตัวถัง 40 พารามิเตอร์
    tele = {
        ...
        "EPS_BATT": f"{random.uniform(92, 98):.1f} %",
        "OBC_TEMP": f"{random.uniform(22, 28):.2f} C",
        "SYS_LOCK": "AES-256",
        "MISSION": "ACTIVE"
    }
    for i in range(32): tele[f"CH_{i+1:02d}"] = f"{random.uniform(10, 80):.2f}"

    # คำนวณวิถีโคจรย้อนหลัง
    lats, lons, alts, vels = [], [], [], []
...
class ENGINEERING_PDF(FPDF):
    def draw_graph(self, x, y, w, h, title, data, color=(0, 70, 180)):
...
def build_pdf(sat_name, addr, s_name, s_pos, s_img, f_id, pwd, m):
    ...
    pdf.set_font("helvetica", '', 10); pdf.cell(0, 8, f"ASSET: {sat_name.upper()} | ID: {f_id}", ln=True, align='C')
    pdf.ln(5)
    items = list(m['RAW_TELE'].items())
    for i in range(0, 40, 4):
        for j in range(4):
            if i+j < len(items): pdf.set_font("helvetica", '', 7); pdf.cell(47.25, 8, f" {items[i+j][0]}: {items[i+j][1]}", border=1)
        pdf.ln()
    pdf.add_page()
    pdf.draw_graph(25, 30, 160, 65, "ORBITAL LATITUDE TRACKING", m['TAIL_LAT'])
    pdf.draw_graph(25, 115, 75, 50, "VELOCITY (KM/H)", m['TAIL_VEL'], (160, 100, 0))
    pdf.draw_graph(110, 115, 75, 50, "ALTITUDE (KM)", m['TAIL_ALT'], (0, 120, 60))
    qr_buf = generate_verified_qr(f_id)
    pdf.image(qr_buf, 20, 190, 40, 40)
    if s_img: pdf.image(BytesIO(s_img.getvalue()), 140, 195, 30, 20)
    pdf.set_xy(110, 220); pdf.set_font("helvetica", 'B', 12); pdf.cell(80, 6, s_name.upper(), align='C', ln=True)
...
with st.sidebar:
    ...
    a1 = st.text_input("Station District", "Phra Nakhon")
...
def archive_dialog():
    if st.session_state.pdf_blob is None:
        ...
        if st.button("🚀 INITIATE", use_container_width=True):
            fid = f"REF-{random.randint(100, 999)}"; pwd = "123456"
            st.session_state.pdf_blob = build_pdf(sat_name, {}, s_name, s_pos, s_img, fid, pwd, run_calculation(sat_catalog[sat_name]))
...
def dashboard():
    ...
    st.markdown(f'''<div style="display:flex; justify-content:center;"><div style="background:white; border:4px solid black; padding:10px 50px; border-radius:100px; text-align:center;"><span style="color:black; font-size:50px; font-weight:900;">{datetime.now(timezone.utc).strftime("%H:%M:%S")} UTC</span></div></div>''', unsafe_allow_html=True)
