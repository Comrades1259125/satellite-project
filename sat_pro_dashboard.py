...
# ตั้งค่าสถานะเพื่อป้องกันป๊อปอัพเด้งมั่วตอนซูม
...
# ==========================================
@st.dialog("📋 MISSION ARCHIVE FINALIZATION")
def archive_dialog(asset):
    if st.session_state.pdf_blob is None:
        pwd = st.text_input("SET PASSWORD", "123456", type="password")
        if st.button("🚀 GENERATE SECURE PDF", use_container_width=True):
        ...
        st.info(f"ID: {st.session_state.m_id} | KEY: {st.session_state.m_pwd}")
...
with st.sidebar:
    ...
    if sat_catalog:
        ...
        st.divider()
        # แถบซูม: ล็อก Key ไว้ทำให้เลื่อนแล้วหน้าไม่เด้ง
        z1 = st.slider("Tactical Zoom", 1, 18, 12, key="z1_slider")
        z2 = st.slider("Global Zoom", 1, 10, 2, key="z2_slider")
        z3 = st.slider("Station Zoom", 1, 18, 15, key="z3_slider")
...
# เรียกป๊อปอัพเฉพาะตอนเปิดสวิตช์ (ล็อกให้ไม่เด้งตอนซูม)
if st.session_state.get('open_sys', False):
    archive_dialog(asset)

# ==========================================
# 5. DASHBOARD (STABLE MAPS)
# ==========================================
...
def dashboard(asset_name):
    ...
    def draw_map(lt, ln, zm, k, tl, tn):
        ...
        fig.update_layout(
            mapbox=dict(
                ...
                center=dict(lat=lt, lon=ln), # แก้ไข: ล็อกพิกัดให้หมุดอยู่กึ่งกลางแผนที่เสมอ
                zoom=zm
            ),
    ...
    st.subheader("🌍 LIVE GEOSPATIAL DATA")
    c1, c2, c3 = st.columns(3)
    with c1: draw_map(m['LAT'], m['LON'], z1, "map1", m["TAIL_LAT"], m["TAIL_LON"])
    with c2: draw_map(m['LAT'], m['LON'], z2, "map2", m["TAIL_LAT"], m["TAIL_LON"])
    with c3: draw_map(13.75, 100.5, z3, "map3", [], []) # ล็อกพิกัดสถานีภาคพื้นดิน
